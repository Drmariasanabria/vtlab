window.NAVAL_MODALS_SYNC = (function () {
  ...
  function dailySeed(date) {
    ...
    return { seed: "DAILY-" + iso, dateKey: iso };
  }
  ...
  function newPlayer(name, avatar, team) {
    return { name, avatar, team: team || null, score: 0, streak: 0, correct: 0, answered: 0, joinedAt: Date.now() };
  }

  function pushReaction(list, playerId, emoji) {
    const next = (list || []).slice(-MAX_REACTIONS + 1);
    next.push({ id: "r_" + Math.random().toString(36).slice(2, 9), playerId, emoji, at: Date.now() });
    return next;
  }
  ...
  LocalAdapter.prototype.getRoom = async function (code) {
    return this._read(code.toUpperCase());
  };
  LocalAdapter.prototype.createRoom = async function (hostName, avatar, team) {
    const code = randomRoomCode();
    const hostId = "p_" + Math.random().toString(36).slice(2, 10);
    const room = { code, status: "lobby", startAt: null, teamsEnabled: !!team, players: { [hostId]: newPlayer(hostName, avatar, team) }, reactions: [] };
    this.code = code;
    this._write(code, room);
    return { code, playerId: hostId };
  };
  LocalAdapter.prototype.joinRoom = async function (code, name, avatar, team) {
    code = code.toUpperCase();
    const room = this._read(code);
    if (!room) throw new Error("NOT_FOUND");
    const playerId = "p_" + Math.random().toString(36).slice(2, 10);
    room.players[playerId] = newPlayer(name, avatar, team);
    this.code = code;
    this._write(code, room);
  ...
  LocalAdapter.prototype.sendReaction = async function (code, playerId, emoji) {
    const room = this._read(code);
    if (!room) return;
    room.reactions = pushReaction(room.reactions, playerId, emoji);
    this._write(code, room);
  };
  ...
  FirebaseAdapter.prototype.getRoom = async function (code) {
    await this.ready;
    const snap = await this.db.ref("rooms/" + code.toUpperCase()).get();
    return snap.exists() ? snap.val() : null;
  };
  FirebaseAdapter.prototype.createRoom = async function (hostName, avatar, team) {
    await this.ready;
    const code = randomRoomCode();
    const ref = this.db.ref("rooms/" + code);
    const hostId = "p_" + Math.random().toString(36).slice(2, 10);
    await ref.set({ status: "lobby", startAt: null, teamsEnabled: !!team, players: { [hostId]: newPlayer(hostName, avatar, team) } });
    return { code, playerId: hostId };
  };
  FirebaseAdapter.prototype.joinRoom = async function (code, name, avatar, team) {
    await this.ready;
    code = code.toUpperCase();
    const ref = this.db.ref("rooms/" + code);
    ...
    const playerId = "p_" + Math.random().toString(36).slice(2, 10);
    await ref.child("players/" + playerId).set(newPlayer(name, avatar, team));
    return { code, playerId };
  };
  ...
  FirebaseAdapter.prototype.sendReaction = async function (code, playerId, emoji) {
    await this.ready;
    await this.db.ref("rooms/" + code + "/reactions").push({ playerId, emoji, at: Date.now() });
  };
  ...
  return {
    ...
    buildSchedule, dailySeed, randomRoomCode, createAdapter, LocalAdapter, FirebaseAdapter
  };
})();
